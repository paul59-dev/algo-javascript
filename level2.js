...
let plusGrand = Math.max(nombre1, nombre2, nombre3); // Prend le plus grand nombre parmi nombre1, nombre2, et nombre3
...
if (note >= 0 && note <= 4) { // Si la note est superieur ou inferieur à 4 alors c'est catastrophique
...
if (!isNaN(nombre6)) {
    // Genere une pyramide d'étoiles avec un nombre de lignes égal au nombre donnée
    for (let i = 1; i <= nombre6; i++) {
        // Genere une chaîne d'espaces en fonction du nombre d'espaces nécessaires
        let espaces = ' '.repeat(nombre6 - i);

        // Genere une chaîne d'étoiles en fonction du nombre d'étoiles nécessaires pour la ligne actuelle
